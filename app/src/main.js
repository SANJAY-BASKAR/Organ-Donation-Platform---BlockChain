...
const App = {
    ...
    register: async function(user) {
        ...
        const phone = user === "Donor" ? document.getElementById(user+'Phone').value : "";
        ...
        if (checkedValues) {
            ...
            if (!validate) {        
                ...
                if (user=="Pledge")
                    ...
                    this.setDonor(fullname, age, gender, medical_id, blood_type, organ, weight, height, phone);
                else if (user=="Patient") 
    ...
    forwardPledge: async function() {
        ...
        await this.contractInstance.methods.getPledge(medical_id).call().then(function(result) {
            ...
            const phone = prompt("Please enter contact number for donor:", "");
            App.setDonor(result[0], result[1], result[2], medical_id, result[3], result[4], result[5], result[6], phone);
        });
    ...
    setDonor: async function(fullname, age, gender, medical_id, blood_type, organ, weight, height, phone) {
        // First set donor without phone
        ...
        // Then set phone separately
        if (phone && phone.length > 0) {
            const gasPhone = await this.contractInstance.methods.updateDonorPhone(medical_id, phone).estimateGas({
                from: this.accounts[0]
            });
            await this.contractInstance.methods.updateDonorPhone(medical_id, phone).send({
                from: this.accounts[0], gas: Math.max(gasPhone, MIN_GAS)
            });
        }
    },
            ...
            try {
                ...
                if (validate) {
                    if (user=="Donor"){
                        ...
                        console.log("Donor data:", result);
                        document.getElementById("search"+user+"Check").innerHTML = null;
                        assignSearchValues(result, user);
                        
                        // Get and display phone separately
                        const phone = await this.contractInstance.methods.getDonorPhone(medical_id).call();
                        console.log("Phone data:", phone);
                        if (document.getElementById("get"+user+"Phone")) {
                            document.getElementById("get"+user+"Phone").innerHTML = "Contact: " + phone;
                        } else {
                            // Create phone element if it doesn't exist
                            const phoneElement = document.createElement("p");
                            phoneElement.id = "get"+user+"Phone";
                            phoneElement.innerHTML = "Contact: " + phone;
                            
                            // Find appropriate parent to append to
                            const resultsContainer = document.getElementById(user.toLowerCase() + "Results") || 
                                                    document.querySelector(".details") ||
                                                    document.getElementById("get"+user+"FullName").parentNode;
                                                    
                            if (resultsContainer) {
                                resultsContainer.appendChild(phoneElement);
                            } else {
                                console.log("Could not find a parent container for search results");
                            }
                        }
                    }
                    else if (user=="Patient") {
                ...
                document.getElementById("search"+user+"Check").innerHTML = "Error in search: " + error.message;
                clearSearchValues(user);
            }
        }
    },
    ...
    viewDonors: async function() {
        ...
        // Create table head once before loop
        if (DonorCount > 0) {
            // Create a sample empty donor to get the keys for the table header
            let sampleDonor = { 
                Index: '', 
                "Full Name": '', 
                Age: '', 
            ...
            let data = Object.keys(sampleDonor);
            generateTableHead(table, data);
        }

        // Process each donor one by one
        for (let i=0; i<DonorCount; i++) {
            try {
                ...
                // Get phone separately
                const phone = await this.contractInstance.methods.getDonorPhone(DonorIDs[i]).call();
